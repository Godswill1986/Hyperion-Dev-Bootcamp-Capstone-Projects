def cypher_message(message):  # Defining a function to cypher a message
    ...
    for char in message:  # Iterating over each character in the input message
        ...
        if char.isalpha():  # Checking if the character is alphabetic
            ...
            cyphered_message += char  # Keep non-alphabetic characters unchanged in the cyphered message
    
    return cyphered_message  # Return the cyphered message

message = "Hello world! Welcome to cyphers." # Test the function with a message

cyphered_message = cypher_message(message)  # Call the cypher_message function

print("\n Encoded message: ", cyphered_message)  # Print the cyphered message

print("\n Plain text message: ", message)  # Print the original plain text message
